...
class Distribusi
{
  ...
  public function addDistribusi($nama_barang, $harga, $jumlah_distribusi, $subtotal, $tujuan, $tanggal_keluar)
  {
    $this->db->conn->begin_transaction();

    try {
      $query1 = "INSERT INTO tb_distribusi (nama_barang, harga, jumlah_distribusi, subtotal, tujuan, tanggal_keluar) VALUES (?, ?, ?, ?, ?, ?)";
      $stmt1 = $this->db->conn->prepare($query1);
      $stmt1->bind_param("siiiss", $nama_barang, $harga, $jumlah_distribusi, $subtotal, $tujuan, $tanggal_keluar);
      $stmt1->execute();
  ...
  {
    $query = "UPDATE tb_distribusi SET nama_barang = ?, jumlah_distribusi = ?, tujuan = ?, tanggal_keluar = ? WHERE id_distribusi = ?";
    $stmt = $this->db->conn->prepare($query);
    $stmt->bind_param("sdssi", $nama_barang, $harga, $jumlah_distribusi, $subtotal, $tujuan, $tanggal_keluar, $id_distribusi);
    return $stmt->execute();
  }
  ...
  public function cekKonfirmasiDistribusi($id_distribusi)
  {
    $query = "SELECT COUNT(*) as total FROM tb_detail_distribusi WHERE id_distribusi = ?";
    $stmt = $this->db->conn->prepare($query);
    $stmt->bind_param("i", $id_distribusi);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    return $data['total'] > 0; // true kalau sudah ada
  }
}
